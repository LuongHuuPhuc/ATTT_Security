...
const int initial_key_permutation[] =  
 {57, 49,  41, 33,  25,  17,  9,
   1, 58,  50, 42,  34,  26, 18,
  10,  2,  59, 51,  43,  35, 27,
  19, 11,   3, 60,  52,  44, 36,
...
void sub_keys_generate(unsigned char *main_key, key_t *key_sets){
  ...
  #pragma omp parallel for private(shift_byte)
  for(i = 0; i < 56; i++){
  ...
  #pragma omp parallel for private(shift_byte, shift_size, first_shift_bits, second_shift_bits, third_shift_bits, fourth_shift_bits)
  for(i = 0; i < 17; i++){
    for(j = 0; j < 4; j++){
    ...
    for(j = 0; j < 48; j++){
      ...
      if(shift_size <= 28){
        ...
        shift_byte &= key_sets[j].d[(shift_size - 29) / 8];
...
void s_box(unsigned char *input48, unsigned char *output32){  
  ...
  for(i = 0; i < 8; i++){
    //Xac dinh hang (row) tu bit 1 va bit 6
    row = ((input48[i / 6] >> (7 - (i % 6))) & 0x02) | ((input48[(i * 6 + 5) / 8] >> (7 - ((i * 6 + 5) % 8))) & 0x01);

    //Xac dinh cot (column)tu 4-bit 
    column = 0;
    column |= ((input48[(i * 6 + 1) / 8] >> (7 - ((i * 6 + 1) % 8))) & 0x01) << 3;
    column |= ((input48[(i * 6 + 2) / 8] >> (7 - ((i * 6 + 2) % 8))) & 0x01) << 2;
    column |= ((input48[(i * 6 + 3) / 8] >> (7 - ((i * 6 + 3) % 8))) & 0x01) << 1;
    column |= ((input48[(i * 6 + 4) / 8] >> (7 - ((i * 6 + 4) % 8))) & 0x01);

    //Truy xuat gia tri tu s-box 
    s_output = S[i][row][column];

    s[i] = s_output;

  }
  //Gop lai thanh mang 4-byte (32-bit)
  memset(output32, 0, 4);
  for(i = 0; i < 8; i++){
    shift_byte = s[i] << 4; //Moi gia tri s-box la 4-bit
    output32[i / 2] |= (shift_byte >> (4 * (i % 2)));
  }
}
...
void process_encryption(unsigned char *plain_text, unsigned char *output_data, key_t *key_sets){
  ...
  unsigned char initial_permutation[8] = {0};
  memset(output_data, 0, 8);

  for(i = 0; i < 64; i++){
    shift_byte = initial_message_permutation[i];
    shift_byte = 0x80 >> ((shift_size - 1) % 8);
    shift_byte &= message_expansion[(shift_byte - 1) / 8];
    shift_byte <<= ((shift_byte - 1) % 8);

    initial_permutation[i / 8] |= (shift_byte >> i % 8);
  }

  unsigned char l[4], r[4], ln[4], rn[4], input_s[6], output_s[4];
  for(i = 0; i < 4; i++){
